...
export const createEtNews = async (req: Request, res: Response): Promise<void> => { 
    ...
    if (!title || title.length > process.env.MAX_CHARACTER) { //Check the limit of title
        res.status(400).json({ message: 'Title is required and must be less than 100 characters.' });
        return;
    }
    if (!content) { //Check the content
        res.status(400).json({ message: 'Content is required.' });
        return;
    }
    if (typeof author !== 'string' || author.length === 0) { //Check the author
        res.status(400).json({ message: 'Author is required and must be a valid string.' });
        return;
    }
    
    if (!publishDate || !isValidDate(publishDate)) { //Check the publish date, BA verify the date is ISO 8601
    ...
    try {
        ...
        if (error.code === 'ETIMEOUT') { //Check the database connection timeout
